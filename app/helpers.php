...
function addArtist($id,$name,$link){
    $list = \App\Models\Artists::where('artist_id',$id)->first();
    if(!$list) {
...
function addTrack($id,$artist,$name,$image,$cdn){
    if(!$image){
        $image = "";
    }
    if(!$cdn){
...
function addGenreArtist($genre,$artist){
    if($genre) {
...
function searchCities($code,$genre){
...
function addGenreList($genres, $artistId){
